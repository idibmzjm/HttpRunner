...
Along with this project, I devised a sample interface service, and you can use it to familiarize how to play with `apiautotest`.
...
Yes! That's what we did in [`requests.request`](requests.request)! Since `apiautotest` takes full reuse of [`Requests`][requests], it inherits all powerful features of [`Requests`][requests], and we can handle HTTP request as the way we do before.
...
However, the test cases are only `YAML` documents, it is impossible to generate parameters dynamically in such text. Fortunately, we can combine `Python` scripts with `YAML/JSON` test cases in `apiautotest`.
...
When we want to reference a variable in the test case, we can do this with a escape character `$`. For example, `$user_agent` will not be taken as a normal string, and `apiautotest` will consider it as a variable named `user_agent`, search and return its binding value.
...
To handle this case, overall `config` block is supported in `apiautotest`. If we define variables or import functions in `config` block, these variables and functions will become global and can be referenced in the whole test set.
...
Reports generated: /Users/Leo/MyProjects/apiautotest/reports/quickstart-demo-rev-0/2017-08-01-16-51-51.html
```
